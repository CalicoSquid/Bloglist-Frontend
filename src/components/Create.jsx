...
export default function Create({ newBlog, setNewBlog, setMessage, setBlogs }) {
  ...
  const handleCreateBlog = async (e) => {
    ...
    try {
      ...
      setMessage((prev) => ({
        ...prev,
        error: error?.response?.data || "Error creating blog",
      }));
      setLoading(false);
      setTimeout(() => {
        setMessage({
  ...
  return (
    <form action="submit" className="create-form flex col">
      <div className="form-fields flex col">
        ...
        <Button
          name="create-button"
          label={!loading ? "Create" : <CircularProgress size={28} color="inherit"/>}
